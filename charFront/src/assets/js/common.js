...
/*关闭选项卡*/
function Closetab(){
	window.parent.$('.page-tabs-content').children("[data-id]").not(":first").each(function () {
	    var flagTxt = $(this).html();
	    /* var temp = "建议概要-B"; */
	    if($(this).hasClass('active'))
	    {
	           $(this).remove();
	           window.parent.$('.page-tabs-content').css("margin-left", "0");
	           window.parent.$('.page-tabs-content').children("[data-id]:first").each(function () {
	       	   window.parent.$('.J_iframe[data-id="' + $(this).data('id') + '"]').show();
